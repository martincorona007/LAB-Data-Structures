...
public:
    ...
    bool search_ABB(nodeType* ptr,int);
...
bool tree::search_ABB(nodeType* ptr, int infor){
    if(infor<ptr->info){
        if(ptr->linkft==NULL){
          //  printf("Information not found");
            return false;
        }else{
            search_ABB(ptr->linkft,infor);
        }
    }else if(infor>ptr->info){
        if(ptr->linker==NULL){
            ...
            search_ABB(ptr->linker,infor);
        }
    }else{
      //  printf("info IS found in the TREE");
            return true;
    }
}
...
int main(){
    ...
    printf("\n\nNumber to found: ");
    cin>>nn;
    res=listt.search_ABB(root,nn);
//    cout.setf(ios::res);
    if( res ?cout<<"true":cout<<"false");
